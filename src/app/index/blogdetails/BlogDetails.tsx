...
const BlogDetails = () => {
  ...
  // Resolve blog: selectedBlog > allBlogs > dummyBlogs
  const blog = useMemo(() => {
    return (
      selectedBlog ??
      allBlogs.find((b) => b.slug === params.slug) ??
      dummyBlogs.find((b) => b.slug === params.slug) ??
      null
    );
  }, [selectedBlog, allBlogs, params.slug]);

  // Fetch blog from backend if not found locally
  useEffect(() => {
  if (!blog) {
    const fetchBlogBySlug = async () => {
      ...
      try {
        ...
        // Map backend fields to your Blog type
        const mappedBlog = {
          id: data._id,
          slug: data.slug,
          title: data.title,
        ...
        setSelectedBlog(mappedBlog); // update context
      } catch (err) {
    ...
    setSelectedBlog(blog); // sync context
  }
  ...
  if (loading) {
    return (
      <div className="p-10 text-center text-gray-600">Loading blog...</div>
    );
  }
