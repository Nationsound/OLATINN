...
import { dummyBlogs } from "../dummy/DummyData"; // your dummy blogs

const API_BASE = "http://localhost:5000/olatinn/api/blogs";

const BlogList = () => {
  const router = useRouter();
  const { setSelectedBlog, allBlogs = [], setAllBlogs } = useBlog(); // allBlogs in context
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        ...
        setAllBlogs(merged); // save in context
      } catch (err) {
        console.error("Error fetching blogs:", err);
        setAllBlogs(dummyBlogs); // fallback
      } finally {
        setLoading(false);
      }
    };
            ...
            >
              ...
              <div className="p-8 flex flex-col justify-between">
                ...
                <div className="mt-6 flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
                  ...
                  <span>
                    {blog.date ?? (blog.createdAt ? new Date(blog.createdAt).toLocaleDateString() : "Unknown date")} •{" "}
        ...
        <div className="flex items-center justify-center gap-4 mt-10">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-full font-semibold transition ${
              page === 1 ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-[#000271] text-white hover:bg-[#17acdd]"
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-full font-semibold transition ${
              page === totalPages ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-[#000271] text-white hover:bg-[#17acdd]"
            }`}
          >
            Next <FaArrowRight />
          </button>
